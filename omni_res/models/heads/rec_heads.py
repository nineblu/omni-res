...
from omni_res.layers.activation import get_activation
...
class REChead(nn.Module):
    ...
    def forward(self, xin, yin,labels=None,y_label=None,mixup_targets=None, imgs=None,):
        # print(labels)
        outputs = []
        origin_preds = []
        x_shifts = []
        ...
        if self.training:
            ...
            # print(pred)
            return pred.view(batchsize,-1),torch.zeros(batchsize,self.hw[0][0]*self.strides[0],self.hw[0][1]*self.strides[0]).to(pred.device)
